...
class Stm():
    ...
    utt_re = re.compile('(.*)\s+(.*)\s+.*\s+(\d+.\d+)\s+(\d+.\d+)\s+<.*>\s+(.*)')
    ...
    def __process_file(self, fin):
        for line in fin:
            ...
            if match:
                ...
                words = unicode(match.group(5), encoding='utf-8').split()
    ...
    def check_truth(self, word, hyp):
        ...
        for utt in hyp:
            for elt in hyp[utt]:
                ...
                if utt.channel:
                    self.mark_truth_with_channel(utt.audio_id, utt.channel, elt, word_idx)
                else:
    ...
    def mark_truth_with_channel(self, audio_id, channel, elt, word_idx):
        ...
        for occurance in self.utts[audio_id][channel]:
            ...
            if percent > max_in:
                elt.correct = True if occurance.contains_word(word_idx) else False
                max_in = percent
                elt.found = True
...
class Utterance():
    ...
    def __str__(self):
        str = "Audio Id: {}, Start: {}, End: {}, Word Ids: {}"
        return str.format(self.audio_id, self.start, self.end, self.word_idxs)
